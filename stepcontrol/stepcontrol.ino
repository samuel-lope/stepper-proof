...
void enviarRespostaParam(uint8_t codigo, uint32_t parametro) {
  if (current_src == SRC_USB) {
    ...
    cmdSerial.print(codigo, HEX);
    cmdSerial.print(':');
    cmdSerial.println(parametro);
  }
}

void enviarRespostaComando(uint8_t codigo, uint8_t id_ou_slot,
                           ComandoMotor cmd) {
  if (current_src == SRC_USB) {
    Serial.print(codigo, HEX);
    Serial.print(':');
    Serial.print(id_ou_slot);
    Serial.print(',');
...
void moverMotor1(uint32_t passos, uint32_t intervalo_us, uint8_t direcao) {
  ...
  m1_em_movimento = 1;
  m1_interval_ticks =
      intervalo_us * 2; // PS=8 gera 0.5us p/ tick (1us = 2 ticks)
  ...
  TIFR1 |=
      (1 << OCF1A); // Limpa qualquer interrupt acidental agendado no passado
...
void inicializarPresetsEEPROM() {
  ...
  ComandoMotor defaults[MAX_PRESETS] = {
      ...
      {3200, 500, 1, 2, 1000,
       1}, // [2] M1, 3200 passos, 500µs, fwd, 2 reps, 1s pausa
      {1600, 400, 1, 1, 0, 2}, // [3] M2, 1600 passos, 400µs, fwd, 1 rep
      {800, 200, 0, 1, 500,
       2},                // [4] M2, 800 passos, 200µs, rev, 1 rep, 500ms pausa
      {0, 0, 0, 0, 0, 1}, // [5] Vazio
      {0, 0, 0, 0, 0, 1}, // [6] Vazio
      {0, 0, 0, 0, 0, 1}, // [7] Vazio
      {0, 0, 0, 0, 0, 1}, // [8] Vazio
...
void constroiEAdicionaComando(ComandoMotor cmd, bool cmd_valido) {
  ...
  if (cmd_valido && cmd.step > 0 && cmd.vel >= 50) {
    ...
    Serial.print(':');
    Serial.println(qtd_comandos_na_fila);
  } else {
...
void interpretarComando(char *linha) {
  ...
  while (par != NULL) {
    ...
    if (chave_str != NULL) {
      ...
      if (valor_str == NULL) {
        ...
        // 0x03 agora requer valor (03:0 ou 03:1), tratado abaixo com os
        // parametros
      } else {
        ...
        if (chave == 0x03) { // repeatAll booleano (03:1 = ON, 03:0 = OFF)
          ...
          eh_parametro = true;
                   ...
                   0x16) { // enableMotor (EN = LOW, ativo baixo TB6600)
          if (valor == 1) {
            PORTD &= ~(1 << M1_EN_PIN);
          } else if (valor == 2) {
            PORTD &= ~(1 << M2_EN_PIN);
          ...
          while (inner_par != NULL) {
            ...
            if (ik && iv) {
              ...
              if (k == 0x10) {
                ...
                preset_cmd.pause_ms = v;
              } else if (k == 0x15) {
                preset_cmd.motor_id = v;
              }
            }
            inner_par = strtok_r(NULL, ",", &ponteiro_virgula);
          }
          if (preset_valido && preset_cmd.vel >= 50) {
            if (preset_cmd.motor_id != 1 && preset_cmd.motor_id != 2)
              preset_cmd.motor_id = 1;
          ...
          return;
        } else if (chave == 0x1B) { // fastActionRep — executa preset EEPROM
                                    // override de repetições (Ex: 1B:slot:rep)
          char *rep_str = strtok_r(NULL, ":", &ponteiro_dois_pontos);
          if (valor > 9 || rep_str == NULL) {
            enviarRespostaHex(0xE4);
            return;
          }
          ...
          return;
        } else if (chave == 0x1C) { // saveToEEPROM - copia slot SRAM para
                                    // EEPROM (Ex: 1C:eeprom_slot:sram_slot)
          char *sram_str = strtok_r(NULL, ":", &ponteiro_dois_pontos);
          if (valor > 9) {
            enviarRespostaHex(0xE4);
            return;
          } // Verifica índice da EEPROM
          if (sram_str == NULL) {
            enviarRespostaHex(0xE3);
            return;
          } // Faltou parâmetro
  ...
  if (eh_parametro) {
    ...
    if (cmd.motor_id != 1 && cmd.motor_id != 2)
      cmd.motor_id = 1;
...
bool carregarProximoComando(uint8_t motor) {
  ...
  bool *infinito_ptr =
      (motor == 1) ? &m1_comando_infinito : &m2_comando_infinito;
  uint16_t *rep_ptr =
      (motor == 1) ? &m1_repeticoes_restantes : &m2_repeticoes_restantes;
  ...
  while ((int)*indice_ptr < (int)qtd_comandos_na_fila) {
    if (fila_comandos[*indice_ptr].motor_id == motor) {
      ...
      *rep_ptr = cmd.repeat;
      ...
      // Telemetria: id customizado p/ diferenciar slots (Ex: M1 linha 2 ->
      // D0:102, M2 linha 3 -> D0:203)
      Serial.print(0xD0, HEX);
      Serial.print(':');
      Serial.println((motor * 100) + *indice_ptr);

...
void parseH8PMessage(const char* msg) {
    ...
    if      (strncmp(msg, "A0", 2) == 0) currentMsg = F("Sis Inicializado");
    else if (strncmp(msg, "B0", 2) == 0) currentMsg = F("Iniciando Fila");
    else if (strncmp(msg, "B1", 2) == 0) currentMsg = F("Motor Parado");
    else if (strncmp(msg, "B2", 2) == 0) currentMsg = F("Pausa Global");
    else if (strncmp(msg, "B4", 2) == 0) currentMsg = F("Repeat ON");
    else if (strncmp(msg, "B5", 2) == 0) currentMsg = F("Fila Executada");
    else if (strncmp(msg, "B6", 2) == 0) currentMsg = F("Repeat OFF");
    else if (strncmp(msg, "B7", 2) == 0) currentMsg = F("Motor Habilitado");
    else if (strncmp(msg, "B8", 2) == 0) currentMsg = F("Motor Desabiltd.");
    else if (strncmp(msg, "B9", 2) == 0 || strncmp(msg, "BA", 2) == 0) currentMsg = F("Preset EEPROM");
    else if (strncmp(msg, "E0", 2) == 0) currentMsg = F("Err: Em Execucao");
    else if (strncmp(msg, "E1", 2) == 0) currentMsg = F("Err: Fila Vazia");
    else if (strncmp(msg, "E2", 2) == 0) currentMsg = F("Err: Fila Cheia");
    else if (strncmp(msg, "E3", 2) == 0) currentMsg = F("Erro de Sintaxe");
    else if (strncmp(msg, "E4", 2) == 0) currentMsg = F("Preset Invalido");
    else if (strncmp(msg, "BB", 2) == 0 || strncmp(msg, "BC", 2) == 0) currentMsg = F("FastAct. Exec");
...
void processKeypad() {
    ...
    if (!key) return; // Se nenhum botão foi pressionado, retorna ao Loop principal
    ...
    if (inputBuffer.endsWith(":")) {
        ...
        else if (key == 'C') { // [* + C] = CLEAR ALL
            inputBuffer = "";
        } 
        else if (key == 'D') { // [* + D] = BACKSPACE
            if (inputBuffer.length() >= 2) inputBuffer.remove(inputBuffer.length() - 2);
            else inputBuffer = "";
        } 
        else if (key == 'A') { // [* + A] = Inserir SINAL DE MENOS (-)
            inputBuffer.setCharAt(inputBuffer.length() - 1, '-');
        } 
        else if (key == '*') {
            if (inputBuffer.length() < MAX_INPUT_LEN) inputBuffer += ":";
        } 
        else {
            if (inputBuffer.length() < MAX_INPUT_LEN) inputBuffer += key;
        }
    } 
    else {

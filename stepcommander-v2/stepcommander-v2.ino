...
char lastCommand[MAX_INPUT_LEN +
                 1];              // Último comando enviado (repetição com *+#)
...
void scrollTextToBuffer(char *out, const char *text, uint8_t textLen,
                        uint16_t idx);
...
void handleDisplayTasks() {
  ...
  if (systemFlags.msgIsStatus && (now - lastMsgTime >= MSG_TIMEOUT_MS)) {
    strcpy_P(currentMsg,
             systemFlags.isFastActMode ? PSTR("[FAST ACT]") : PSTR("Pronto."));
    systemFlags.isMessageLong = 0;
    systemFlags.msgIsStatus = 0;
    scrollIndexBottom = 0;
    systemFlags.lcdNeedsUpdate = 1;
  ...
  if (systemFlags.isMessageLong) {
    ...
    uint16_t interval =
        (uint16_t)max(200, SCROLL_INTERVAL_BASE - (int)(msgLen * 10));
